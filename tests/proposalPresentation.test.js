...
test('restored proposal remains responsive, keyboard-visible, and motion-safe', () => {
  ...
  assert.doesNotMatch(experience, /ChevronLeft|ChevronRight|proposal-mobile-nav-arrow/)
  assert.match(experience, /\{activeIndex \+ 1\} of \{proposal\.sections\.length\}/)
  assert.match(experience, /proposal-mobile-tab-step.*index < activeIndex \? ' completed'/s)
  assert.match(experience, /className="proposal-mobile-tab-connector"/)
  assert.match(styles, /\.proposal-mobile-tabs\s*\{[^}]*overflow-x:\s*auto/s)
  assert.match(styles, /\.proposal-mobile-tab-step\.completed \.proposal-mobile-tab-connector\s*\{[^}]*background:\s*#0b5fa5/s)
