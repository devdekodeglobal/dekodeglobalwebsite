...
test('restored proposal remains responsive, keyboard-visible, and motion-safe', () => {
  ...
  assert.match(experience, /activeIndex \/ \(proposal\.sections\.length - 1\)/)
  assert.match(experience, /index <= activeIndex \? 'completed'/)
  assert.match(experience, /index === activeIndex \? 'current'/)
  assert.match(styles, /\.proposal-mobile-tabs\s*\{[^}]*overflow-x:\s*auto/s)
  assert.match(styles, /\.proposal-mobile-progress-fill\s*\{[^}]*background:\s*#0b5fa5/s)
